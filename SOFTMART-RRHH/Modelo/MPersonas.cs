...
namespace SOFTMART_RRHH.Controlador
{
    internal class MPersonas
    {
        ...
        internal static DataTable ObtenerPersonaInformacionByCURP(string CURP)
        {
            ...
            return LibAux.EjecutarSentencia("SELECT * FROM  " +
                "Personas p " +
                "LEFT JOIN Escolaridades e ON e.idEscolaridad = p.idEscolaridad " +
                "INNER JOIN Domicilios d ON p.idDomicilio = d.idDomicilio  " +
                "INNER JOIN Contactos c ON c.idContacto = p.idContacto AND p.CURP LIKE @vCURP ;", parametros);
        }
